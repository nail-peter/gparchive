...
function playEpisode(episode) {
    ...
    const savedPosition = getSavedPosition(episode.filename);
    if (savedPosition > 0) {
        // Set a flag to restore position after metadata loads
        audioPlayer.addEventListener('loadedmetadata', function restorePosition() {
            // Only restore if not near the end (within last 30 seconds)
            if (savedPosition < audioPlayer.duration - 30) {
...
function savePosition(filename, currentTime) {
    try {
        const key = `position_${filename}`;
...
function getSavedPosition(filename) {
    try {
        ...
        console.warn('Could not get saved position from localStorage:', e);
        return 0;
    }
}
...
audioPlayer.addEventListener('timeupdate', () => {
    if (currentEpisode && audioPlayer.currentTime > 0) {
        ...
        if (Math.floor(audioPlayer.currentTime) % 5 === 0) {
            savePosition(currentEpisode.filename, audioPlayer.currentTime);
        }
    }
});

// Clear position when episode ends
audioPlayer.addEventListener('ended', () => {
    if (currentEpisode) {
        savePosition(currentEpisode.filename, 0);
    }
});
